...
public class TicketController {
    ...
    public TicketController(UserRepository userRepository, TicketRepository ticketRepository,
                            ...
                            TicketStatusRepository ticketStatusRepository, UserService userService, TicketService ticketService , DashboardService dashboardService, TicketCommentRepository ticketCommentRepository) {
    ...
    public String createTicket(@RequestParam("title") String title, @RequestParam("description") String description,
        ...
        try {
            ...
            User createdBy = userOptional.orElseThrow(() ->
    ...
    public String filterTickets(
        ...
        List<Ticket> tickets = ticketRepository.searchTicketsWithFilters(
                statusId, priorityId, typeId, assignedToId,
    ...
    public String createTicketByAdmin(@RequestParam String title,
    ...
    public String updateTicket(@ModelAttribute("ticketToEdit") Ticket updatedTicket,
        ...
        try {
            ...
            TicketStatus newStatus = ticketStatusRepository.findById(updatedTicket.getStatus().getStatusId()).orElse(null);

            // On vérifie si le nouveau statut est "Resolved" et si le ticket n'a pas déjà une date de résolution
            if (newStatus != null && "Resolved".equalsIgnoreCase(newStatus.getStatusName()) && ticketToSave.getResolvedAt() == null) {
            ...
            ticketToSave.setStatus(ticketStatusRepository.findById(updatedTicket.getStatus().getStatusId()).orElse(null));
            ticketToSave.setPriority(ticketPriorityRepository.findById(updatedTicket.getPriority().getPriorityId()).orElse(null));
    ...
    public String assignTicket(
            // @RequestParam lie un paramètre de la requête (du formulaire) à un argument de la méthode.
            ...
            // 'defaultValue = "false"' garantit que la variable 'notifyTechnician' sera 'false' dans ce cas.
            // Si elle est cochée, elle enverra sa 'value' ("true"), que Spring convertira correctement.
            @RequestParam(value = "notifyTechnician", defaultValue = "false") boolean notifyTechnician,

            // RedirectAttributes est utilisé pour passer des messages à la page suivante après une redirection.
            RedirectAttributes redirectAttributes
    ) {
        // Étape 1 : Créer l'objet DTO (l'"enveloppe") pour passer les données proprement au service.
        ...
        // Étape 2 : Appeler le service dans un bloc try-catch pour gérer les erreurs éventuelles.
        try {
            ...
            redirectAttributes.addFlashAttribute("successMessage", "Ticket #" + ticketId + " a été assigné avec succès !");
        } catch (Exception e) {
            // Si une erreur survient (ex: ticket non trouvé), on prépare un message d'erreur.
